...
public class Evaluator {
	...
	public static void main(String[] args) {
		...
		if(args.length < 2)
		{
            ...
            System.out.println("\t[ -col_sample <f> ]\tFeature sampling when spliting a node (default=" + LambdaMART.col_sample + ").");
            System.out.println("\t[ -row_sample <f> ]\tInstance sampling when creating a tree (default=" + LambdaMART.row_sample + ").");
            System.out.println("\t[ -alpha <f> ]\t  risk-sensitivity parameter that controls the tradeoff between risk and reward (default=" + LambdaMART.alpha + ").");
            System.out.println("\t[ -risk_type <t> ]\t  risk-sensitivity type urisk:0 -- SARO:1 -- FARO:2  (default=" + LambdaMART.risk_type + ").");
			System.out.println("");
		...
		for(int i=0;i<args.length;i++)
		{
            ...
            else if(args[i].compareTo("-col_sample") == 0)
            {
                LambdaMART.col_sample = Float.parseFloat(args[++i]);
            }
            else if(args[i].compareTo("-row_sample") == 0)
            {
                LambdaMART.row_sample = Float.parseFloat(args[++i]);
            }
            else if(args[i].compareTo("-alpha") == 0)
            {
                LambdaMART.alpha = Float.parseFloat(args[++i]);
            }
            else if (args[i].compareTo("-risk_type") == 0)
            {
                LambdaMART.risk_type = Integer.parseInt(args[++i]);
            }
			//for debugging
